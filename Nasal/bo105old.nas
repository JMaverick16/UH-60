...
var vibration = { # and noise ...
	...
	update: func(dt) {
		...
		if (airspeed > 141) { # overspeed vibration
			...
			var v = 0.49 + 0.5 * normatan(airspeed - 156, 10);
			var intensity = v;
...
var init_weapons = func {
	...
	setlistener("/sim/model/bo105/weapons/impact/HOT", func(n) {
		...
		geo.put_model("Aircraft/UH-60/Models/hot.ac", impact,
		...
		fgcommand("play-audio-sample", props.Node.new({
			#path : getprop("/sim/fg-root") ~ "/Aircraft/UH-60/Sounds",
			file : "HOT.wav",
			volume : 0.2,
		}));
	});
...
#aircraft.livery.init("Aircraft/OH-6/Models/Liveries", "sim/model/bo105/name");
